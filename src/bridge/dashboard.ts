            ...
            nodes.forEach(function(node, i) {
                ...
                if (node.userId && node.userId > 0) {
                    ...
                    var img = svgEl('image', {
                        ...
                        href: '/api/avatar?userId=' + encodeURIComponent(node.userId),
        ...
        function renderClients(clients) {
            ...
            clientList.innerHTML = clients.map(function(c) {
                ...
                if (c.userId && c.userId > 0) {
                    avatarContent = '<img src="/api/avatar?userId=' + encodeURIComponent(c.userId) + '" data-initials="' + escapeHtml(getInitials(c.username)) + '" />';
                } else {
                    avatarContent = escapeHtml(getInitials(c.username));
                }
                return '<div class="client-card">' +
                    '<div class="client-avatar">' + avatarContent + '</div>' +
                    '<div class="client-info">' +
                        '<div class="client-name-row">' +
                            '<span class="client-username">' + escapeHtml(c.username) + '</span>' +
                            '<span class="client-transport ' + transportClass + '">' + escapeHtml(c.transport.toUpperCase()) + '</span>' +
                        '</div>' +
                        '<div class="client-place">' + escapeHtml(c.placeName) + '</div>' +
                        '<div class="client-id">' + escapeHtml(c.clientId) + '</div>' +
                    '</div>' +
                '</div>';
            }).join('');
